...
class Gui():
    ...
    def __init__(self, coords):
        ...
        #start_pygame_application
        pygame.init()
        self.win = pygame.display.set_mode((self.width, self.width))
        self.clock = pygame.time.Clock()
        pygame.display.set_caption("Pathfinding Algorithms - Arzoo")
    ...
    # removing nodes such as walls checkpoints etc
    def remove(self):
        coords = self.get_box_coords()
...
class CoOrdinates():
    ...
    def create_maze(self, Gui):
        ...
        # make sure the size of the maze is either the size of the gui
        ...
        self.maze = [(0 for x in range(largest))
...
def pathfind(maze, start, end, Gui, coords, key):
    ...
    while len(open_list) > 0:
        # skip pathfinding to create a wait effect. Adjustable speed
        if count >= Gui.animation_speed:
            ...
            for new_pos in [(-1,0), (0,1), (1,0), (0,-1)]: # Adjacent squares
                ...
                # for dfs and bfs we don't add anything to the node values
                ...
                if key == "e": # dijkstra, add one to g value
                    ...
                    # it makes it prioritise diagonal paths over straight ones
                    ...
                    child.h = (((abs(child.position[0] - end_node.position[0]) ** 2) +
                                (abs(child.position[1] - end_node.possition[1]) * 2)) * 0.6)
...
class Node():
    ...
    def _init_(self, parent, position):
        self.parent = parent
